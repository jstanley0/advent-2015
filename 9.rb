...
def find_best_path(graph, from_city, visited_cities = [], dist_so_far = 0)
  ...
  graph[from_city].map { |next_hop, dist|
    find_best_path(graph, next_hop, visited_cities, dist_so_far + dist) unless visited_cities.include?(next_hop)
  }.compact.min
end

puts graph.keys.map { |city| find_best_path(graph, city) }.compact.min
